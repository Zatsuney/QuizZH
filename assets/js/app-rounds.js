...
async function displayCurrentQuestion() {
  ...
  // Vérifier le type de question (multichoice ou texte)
  const isMultiChoice = question.answers && question.answers.length > 0;

  if (isMultiChoice) {
    // Afficher les options de réponse (choix multiples)
    if (answersContainer) {
      answersContainer.innerHTML = '';
      question.answers.forEach((answer, index) => {
        const optionDiv = document.createElement('label');
        optionDiv.className = 'answer-option' + (answerValue === answer.text ? ' selected' : '');
        optionDiv.style.cursor = previousAnswer?.answer ? 'not-allowed' : 'pointer';
        
        const radio = document.createElement('input');
        radio.type = 'radio';
        radio.name = 'answer';
        radio.value = answer.text;
        radio.disabled = !!previousAnswer?.answer;
        radio.checked = answerValue === answer.text;
        ...
        // Add click handler to update selection visual state
  ...
  // Désactiver/Réactiver le bouton soumettre
  if (answerForm) {
...
async function submitAnswer() {
  ...
  // Déterminer si c'est une question multichoice ou texte
  if (selectedRadio) {
    // Question multichoice
    answer = selectedRadio.value;
  } else if (answerInput && answerInput.style.display !== 'none') {
    // Question texte
    answer = answerInput.value.trim();
  }

  if (!answer) {
    showNotification('Veuillez sélectionner ou entrer une réponse!', 'error');
    return;
  }
  ...
  try {
    ...
    // Désactiver les options de réponse, le bouton et l'input
    document.querySelectorAll('input[name="answer"]').forEach(radio => {
      radio.disabled = true;
    });
    document.querySelectorAll('.answer-option').forEach(opt => {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Setup enter key to submit (for text input questions)
