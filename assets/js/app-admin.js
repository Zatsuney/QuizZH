...
async function updatePlayersDisplay() {
  try {
    ...
    // Filter only tournament players (not solo players)
    const tournamentPlayers = Object.values(allPlayers).filter(p => p.gameMode === 'tournament' || !p.gameMode);
    const displayPlayers = tournamentPlayers.slice(0, 20);
    
    const totalPlayers = tournamentPlayers.length;
    const waitingPlayers = tournamentPlayers.filter(p => p.status === 'waiting').length;
    const playingPlayers = tournamentPlayers.filter(p => p.status === 'playing').length;

    console.log(`Tournament Players - Total: ${totalPlayers}, Waiting: ${waitingPlayers}, Playing: ${playingPlayers}`);
